...
public class LongRunningTask implements Callable<LongRunningTask.TaskResponse> {
    ...
    public TaskResponse call() throws InterruptedException {
        ...
        while (numSecs++ < time) {
            ...
            // we specifically check the interrupt status so that we are responsive during cancellation
            // Threads.interrupted() method will clears the interrupt status flag, but this is ok because we are throwing exception right after this
            ...
            try {
                // the thread will wake up from the sleep and threw an InterruptedException if is get interrupted
                Thread.sleep(Duration.ofSeconds(1));
                // new HttpCaller(name).makeCall(1);
            }
            catch (InterruptedException intExp) { // this should be the part to exit or terminate thread
                throwInterruptedException();
            }

            // process data (Code not shown) which uses CPU for 0.2 secs
            // during next loop it will go to check the thread is it interrupted to decide to throw an interrupted exception
        }
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        try (ExecutorService service = Executors.newFixedThreadPool(2)) {
            ...
            // interrupt method is call on the thread which is running the task will make the task immediately wake up from the sleep
            taskFtr.cancel(true);
        }
